...
<title><?= e($page_title) ?> - نموذج ج</title>
...
.h-banner { background:linear-gradient(135deg,#0f172a,#1e293b); border-radius:24px; padding:22px 30px; color:#fff; display:flex; justify-content:space-between; align-items:center; gap:14px; flex-wrap:wrap; box-shadow:0 18px 36px rgba(15,23,42,.18); margin-bottom:14px; }
.hb-left h1 { font-size:23px; font-weight:900; margin:0 0 5px; display:flex; align-items:center; gap:9px; }
.hb-left p { font-size:12px; color:#cbd5e1; margin:0; font-weight:700; }
.status-pill { padding:7px 18px; border-radius:99px; font-weight:900; font-size:12.5px; background:var(--pc); box-shadow:0 0 16px var(--pc)66; }

.strip { display:flex; gap:10px; margin-bottom:18px; overflow-x:auto; padding-bottom:4px; }
.pill { background:#fff; border:1px solid var(--border); border-radius:99px; padding:9px 18px; font-size:12px; font-weight:800; white-space:nowrap; display:flex; align-items:center; gap:7px; flex-shrink:0; box-shadow:0 2px 6px rgba(0,0,0,.03); }
.pill i { font-size:13px; }
.pill.clickable { cursor:pointer; transition:.2s; }
.pill.clickable:hover { transform:translateY(-2px); box-shadow:0 6px 14px rgba(0,0,0,.08); }
...
.bento-h { font-size:14px; font-weight:900; margin:0 0 16px; display:flex; align-items:center; gap:9px; }
...
.act-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(160px,1fr)); gap:10px; }
.act-btn { padding:14px; border-radius:13px; border:none; font-size:13.5px; font-weight:900; color:#fff; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px; transition:.2s; }
...
.upload-area { border:2px dashed #cbd5e1; border-radius:14px; padding:16px; text-align:center; cursor:pointer; position:relative; margin-top:12px; max-width:400px; }
...
.fda-panel { display:none; margin-top:14px; }
.fda-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:12px; }
.fda-kpi { background:#fff; border:1px solid #bae6fd; border-radius:14px; padding:16px; text-align:center; }
.fda-kpi .v { font-size:24px; font-weight:900; }
.fda-kpi .l { font-size:10.5px; font-weight:800; margin-top:4px; }

.tl-item { display:flex; gap:14px; margin-bottom:18px; position:relative; }
.tl-item::after { content:''; position:absolute; left:18px; top:36px; bottom:-18px; width:2px; background:var(--border); }
.tl-item:last-child::after { display:none; }
.tl-icon { width:36px; height:36px; border-radius:50%; background:#f8fafc; border:2px solid var(--border); display:flex; align-items:center; justify-content:center; color:var(--muted); z-index:2; flex-shrink:0; font-size:13px; }
...
.prev-item { display:flex; align-items:center; gap:8px; background:#f8fafc; border:1px solid var(--border); border-radius:9px; padding:8px 12px; text-decoration:none; margin-bottom:6px; }
...
<div class="h-banner">
    <div class="hb-left">
        ...
        <p><?= e($c['dept_name'] ?? '—') ?> · <?= e($c['requester_name'] ?? '—') ?> · <span class="eng"><?= date('d M Y', strtotime($c['created_at'])) ?></span></p>
    </div>
...
<div class="strip">
    <?php if ($c['asset_id']): ?>
    <div class="pill"><i class="fa-solid fa-hourglass-half" style="color:#64748b"></i> العمر: <span class="eng"><?= e($asset_age_txt ?? '—') ?></span></div>
    <div class="pill" style="background:<?= $warranty_info && $warranty_info['active'] ? '#dcfce7':'#fee2e2' ?>;color:<?= $warranty_info && $warranty_info['active'] ? '#15803d':'#b91c1c' ?>"><i class="fa-solid fa-shield-halved"></i> <?= $warranty_info ? ($warranty_info['active']?'ضمان ساري':'ضمان منتهٍ') : 'بلا ضمان' ?></div>
    <div class="pill"><i class="fa-solid fa-tag" style="color:#64748b"></i> <span class="eng"><?= e($c['tag_number'] ?: '—') ?></span></div>
    <div class="pill clickable" onclick="document.getElementById('histPanel').scrollIntoView({behavior:'smooth'})"><i class="fa-solid fa-clock-rotate-left" style="color:#9333ea"></i> <span class="eng"><?= $prev_total ?></span> بلاغ سابق</div>
    <div class="pill clickable" style="background:#eff6ff;color:#1d4ed8" onclick="toggleFda()"><i class="fa-solid fa-globe"></i> FDA <i class="fa-solid fa-chevron-down" id="fdaChev" style="font-size:9px"></i></div>
    <?php endif; ?>
</div>

<div class="bento fda-panel" id="fdaPanel">
    <div class="bento-h"><i class="fa-solid fa-globe"></i> السجل العالمي لسلامة الجهاز (FDA)</div>
    <div id="fdaLoading" style="font-size:12.5px;font-weight:800;color:#0e7490">جاري الاستدعاء...</div>
...
<?php if (!in_array($c['status'], ['closed','cancelled','rejected'])): ?>
<div class="bento">
    <div class="bento-h"><i class="fa-solid fa-bolt"></i> لوحة التحكم الفنية</div>
    <form method="POST" id="actForm">
        ...
        <div class="act-grid">
            ...
            <?php if ($c['status']==='in_progress' && $can_edit): ?><button type="button" class="act-btn" style="background:#fff;color:#b45309;border:2px solid #fde68a" onclick="toggleBox('stallBox')"><i class="fa-solid fa-pause"></i> تعثّر</button><?php endif; ?>
            ...
            <?php if (in_array($c['status'],['open','acknowledged']) && $can_manage): ?><button type="button" class="act-btn" style="background:#fff;color:#64748b;border:2px solid #e2e8f0" onclick="toggleBox('rejBox')"><i class="fa-solid fa-ban"></i> رفض</button><?php endif; ?>
            <?php if ($can_manage): ?><button type="button" class="act-btn" style="background:#475569" onclick="if(confirm('إغلاق إداري مباشر. متأكد؟')) doAct('close')"><i class="fa-solid fa-lock"></i> إغلاق</button><?php endif; ?>
        </div>
...
<?php if ($prev_complaints): ?>
<div class="bento" id="histPanel">
    <div class="bento-h"><i class="fa-solid fa-clock-rotate-left"></i> بلاغات سابقة على هذا الجهاز</div>
...
let fdaOpen = false, fdaLoaded = false;
async function toggleFda() {
    fdaOpen = !fdaOpen;
    document.getElementById('fdaPanel').style.display = fdaOpen ? 'block' : 'none';
    document.getElementById('fdaChev').style.transform = fdaOpen ? 'rotate(180deg)' : 'rotate(0)';
    if (fdaOpen) document.getElementById('fdaPanel').scrollIntoView({behavior:'smooth'});
    if (fdaLoaded || !fdaOpen) return;
    fdaLoaded = true;
    ...
    try {
        ...
        if (d.fda_stats && d.fda_stats.total>0) {
            let html=`<div class="fda-kpi"><div class="v eng" style="color:#0e7490">${d.fda_stats.total.toLocaleString()}</div><div class="l" style="color:#0e7490">بلاغ مسجَّل</div></div>`;
            if (!d.fda_stats.is_fallback && d.fda_stats.malfunction!==null) {
                html+=`<div class="fda-kpi"><div class="v eng" style="color:#dc2626">${d.fda_stats.malfunction.toLocaleString()}</div><div class="l" style="color:#b91c1c">عطل مصنعي</div></div><div class="fda-kpi"><div class="v eng" style="color:#d97706">${d.fda_stats.injury_death.toLocaleString()}</div><div class="l" style="color:#b45309">تحذير خطورة</div></div>`;
            } else { html+=`<div class="fda-kpi" style="grid-column:span 2;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:#0f766e">آمن ضمن قواعد السلامة</div>`; }
            wrap.innerHTML=html; wrap.style.display='grid';
        } else { load.style.display='block'; load.textContent='لا توجد بيانات مطابقة لهذا الجهاز.'; }
    } catch(e) { load.style.display='block'; load.textContent='تعذّر الاتصال بقاعدة FDA.'; }
}
</script>
</body>
</html>