<?php
/**
 * reports/maintenance/wo_by_status.php — حسب حالة أمر العمل
 */
require_once dirname(__DIR__, 2) . '/config.php';
require_once dirname(__DIR__, 2) . '/includes/report_helpers.php';
page_guard('reports.maintenance.wo_by_status');

$can_export  = can('reports.maintenance.wo_by_status', 'export');
$excel_mode  = report_excel_mode_active('reports.maintenance.wo_by_status');
$print_mode  = report_print_mode_active('reports.maintenance.wo_by_status');
$print_charts = report_print_charts_mode_active('reports.maintenance.wo_by_status');

$rtl = is_rtl();
$page_title = $rtl?'حسب حالة أمر العمل':'By Work Order Status';
...
$rows = $pdo->query("
    SELECT status, wo_type, COUNT(*) AS cnt,
           AVG(work_hours_total) AS avg_hrs,
           AVG(DATEDIFF(COALESCE(actual_completion_date, CURDATE()), wo_date)) AS avg_days
    FROM complaint_work_orders
    GROUP BY status, wo_type
    ORDER BY status, wo_type
...
$STATUS_AR = ['draft'=>'مسودة','sent_to_contractor'=>'مرسلة للمقاول','in_progress'=>'قيد التنفيذ','pending_manager_approval'=>'بانتظار موافقة','completed'=>'مكتملة','rejected_by_manager'=>'مرفوضة','cancelled'=>'ملغاة'];
...
<div class="container">
    ...
    <div class="hero">
        ...
        <div>
            <h1><?= $rtl?'حسب حالة أمر العمل':'By Work Order Status' ?></h1>
            <div style="margin-top:8px">
                <?= report_export_buttons('reports.maintenance.wo_by_status') ?>
            </div>
            <p><?= $rtl?'التوزيع التفصيلي: حالة × نوع + متوسط ساعات العمل + متوسط الأيام':'Detailed: status × type + avg work hours + avg days' ?></p>
        </div>
    ...
    <div class="sec">
        <div class="sec-h">
            ...
            <?= $rtl?'التفصيل حسب الحالة/النوع':'Detail by Status/Type' ?>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th><?= $rtl?'متوسط الساعات':'Avg Hours' ?></th>
                    <th><?= $rtl?'متوسط الأيام':'Avg Days' ?></th>
                </tr>
            </thead>
            <tbody>
            <?php if (!$rows): ?>
            ...
            ?>
                <tr>
                    ...
                    <td><?= e($TYPE_AR[$r['wo_type']] ?? $r['wo_type']) ?></td>
                    <td><strong><?= (int)$r['cnt'] ?></strong></td>
                    <td><?= $r['avg_hrs'] !== null ? round((float)$r['avg_hrs'], 1).'h' : '—' ?></td>
                    <td><?= $r['avg_days'] !== null ? round((float)$r['avg_days'], 1).'d' : '—' ?></td>
                </tr>
